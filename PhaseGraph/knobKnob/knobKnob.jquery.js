...
var pressureScale = d3.scale.linear().domain([0, 360]).range([50, 150]);
	...
	$.fn.knobKnob = function(props){
		...
		return this.each(function(){
			...
			if(options.value > 0 && options.value <= 359){
				...
				options.turn(currentDeg/359,currentDeg);
			}
			...
			knob.on('mousedown touchstart', function(e){
				...
				doc.on('mouseup.rem  touchend.rem',function(event){
					knob.off('.rem');
					doc.off('.rem');
					...
					if(topPos > 400){
						...
						updateSlider(xScale(currentDeg));
					}
					//changeFlame(xScale(currentDeg));
